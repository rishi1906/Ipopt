...
\item\label{it:cyg_inst} Now you just follow the instructions in the
  ...
  \texttt{bin} (called ``\texttt{ipopt.exe}'').  If you want to test
...
equivalent \verb|C:\Progra~1\Java| (or similar).
...
compiler flag. While per default, an Ipopt shared library is compiled with 
this flag, for a configuration of \Ipopt in debug mode ({\tt --enable-debug}) 
or as static library ({\tt --disable-shared}), the configure flag {\tt --with-pic}
need to be used to enable compilation with {\tt -fPIC}.
...
If you let this method return {\tt false}, \Ipopt will terminate
